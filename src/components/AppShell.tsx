...
import {
  ...
  Menu, X, Crown, User, TrendingUp, DollarSign, Globe
} from "lucide-react";
...
export default function AppShell({ children }: AppShellProps) {
  ...
  // ── Full-width layout (landing, auth, pricing) ─────────────────────────────
  if (isFullWidth) {
    return (
      <>
  ...
  // ── Sidebar layout (dashboard, opportunities, signals) ─────────────────────
  const navItems = [
      ...
      <aside className={cn(
          ...
          {navItems.map((item) => {
            ...
            return (
              <button key={item.path}
                ...
                {item.label}
                {item.path === "/signals" && (
                  <span className="ms-auto text-[10px] bg-primary/20 text-primary font-mono px-1.5 py-0.5 rounded">
                    NEW
                  </span>
                )}
              </button>
        ...
        <div className="p-3 border-t border-border space-y-2">
          ...
          {user ? (
            ...
            <Button variant="default" size="sm" className="w-full" onClick={() => navigate("/login")}>
              Sign In
            </Button>
          )}
        </div>
